...
% Definir Matrices.
n = length(x)+2; % Numero de columnas.
m = length(y)+1; % Numero de renglones.
...
materror = ones(m-2,n-2); % Error permitido, almacenado en una matriz
...
%hgraf(1,:)=[]; % Eliminar primera fila.

% Generar gráfica 
figure(1)
surface(x,y,hgraf) % Instruccion que gráfica.
shading interp
xlabel('x')
ylabel('y')
zlabel('h(x,y)')
